...
import glob
import cv2 as cv
import silhouetteReconstruction
...
def set_voxel_positions(width, height, depth):
    ...
    grid = silhouetteReconstruction.FinalGrid
    data, colors = [], []
    for i in range(len(grid.state)):
        if grid.state[i] == 1:
            [x,y,z] = grid.coordinates[i] 
...
def get_cam_positions():
    ...
    translations = translations / 60 # to make it fit on the scene
